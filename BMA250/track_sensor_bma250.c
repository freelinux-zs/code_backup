...
#define WHOISYOUDADBMA		"MANWANGDAKE_SHENZHEN_LEGENDTIMES"
...
static void sensor_bma250_init_with_interrupt(void)
{
   ...
   bma250_set_slope_threshold(track_sensor_config_data.sensor_threshold);
...
static void acc_sensor_get_6bytes(kal_uint16 *accl_x,kal_uint16 *accl_y,kal_uint16 *accl_z)
{
   bma250acc_t accl;
   ...
   *accl_z=accl.z;
   ...
   LOGZ("bma 250 value %d %d %d", *accl_x,*accl_y,*accl_z);
}


void acc_sensor_get_6bytes_01(kal_uint16 *accl_x,kal_uint16 *accl_y,kal_uint16 *accl_z)
{
   bma250acc_t accl;
...
static void track_sensor_bma250_data_self_test(void)
{
    ...
    if ((bw!=bma250_BW_125HZ )||(range!=3)||(mode!=3)||(threshold != track_sensor_config_data.sensor_threshold))
    {
        kal_prompt_trace(MOD_TST,"track_sensor_bma250_data_self_test Sensor data error (%d-%d-%d-%d) !=[12-3-3-%d]",bw,range,mode,threshold,track_sensor_config_data.sensor_threshold);
        tr_stop_timer(MOTION_SENSOR_DATA_TIMER_ID);
        tr_start_timer(MOTION_SENSOR_RESET_TIMER_ID,  3000, track_sensor_bma250_reset);
    }
    tr_start_timer(MOTION_SENSOR_DATA_TIMER_ID, 60000, track_sensor_bma250_data_self_test);
}
...
static void track_sensor_bma250_reset(void)
{
    ...
    acc_sensor_pwr_down();
    tr_start_timer(MOTION_SENSOR_RESET_TIMER_ID,  10000,sensor_bma250_init_with_interrupt);
}
...
static void track_sensor_bma250_interrput_configuration(void)
{
    ...
    if (!track_is_timer_run(MOTION_SENSOR_DATA_TIMER_ID))
    {
        tr_start_timer(MOTION_SENSOR_DATA_TIMER_ID, 10000, track_sensor_bma250_data_self_test);
    }
}
...
void track_sensor_bma250_on(kal_uint8 threshold , FuncPtr callback)
{
	...
	track_drv_Set_VMC(KAL_TRUE, 1);  //vmem 上电
...
void track_sensor_bma250_off(void)
{
    ...
    tr_stop_timer(MOTION_SENSOR_DATA_TIMER_ID);
    tr_stop_timer(MOTION_SENSOR_RESET_TIMER_ID);    
    acc_sensor_pwr_down();
 ...
 * modification history
 * ----------------------------------------
 * v1.0  , 26-02-2013,    written
 * ----------------------------------------
 ******************************************************************************/
kal_int8 track_sensor_bma250_type_check(void)
{
    bma250_t  bma250_check;

    GPIO_ModeSetup(ACC_SENSOR_SCK, 0x00);
    GPIO_ModeSetup(ACC_SENSOR_SDA, 0x00);
    
    bma250_init(&bma250_check);
            
    if (bma250_check.chip_id==0x03) //BMA250
    {
        return 1;
    }
    else if (bma250_check.chip_id==0xF9) //BMA250E
    {
        return 2;
    }
    else
